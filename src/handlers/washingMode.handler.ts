...
const getAll: Handler<WashingModeListDto, { Querystring: WashingModeQueryDto }> = async (req, res) => {
    try {
        ...
        // Get washing modes - selecting all fields that match the DTO
        const washingModes = await prisma.washingMode.findMany({
            ...
            select: {
                ...
                price: true,
                isActive: true,
                // Add these fields to match the DTO
                duration: true,
                capacity: true,
            },
            orderBy: {
        ...
        // If the duration and capacity fields don't exist in the database,
        // we need to add them to each result
        const formattedWashingModes = washingModes.map((mode) => ({
            ...mode,
            duration: mode.duration ?? 0, // Provide defaults if these fields don't exist
            capacity: mode.capacity ?? 0,
        }));
        ...
        return res.send({
            washingModes: formattedWashingModes,
            pagination: {
                total,
                page,
                limit,
