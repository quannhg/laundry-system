...
import { LaundryStatus, OrderStatus, PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();
const SALT_ROUNDS = 10;


interface WashingModeData {
    id: string;
    name: string;
    price: number;
}
...
async function generateWashingModes() {
    const washingModes: WashingModeData[] = [
        {
            id: 'mode-normal',
            name: 'Normal',
            price: 25000, // VND
    ...
    await prisma.washingMode.createMany({
        data: washingModes as any,
    });
    console.log(`${washingModes.length} washing modes created`);
    return washingModes;
}
...
async function generateOrders(washingModes: WashingModeData[]) {
    const paymentMethods = ['Credit Card', 'visa', 'Mobile Banking'];
    const normalModeId = washingModes.find(mode => mode.name === 'Normal')!.id;
    const thoroughlyModeId = washingModes.find(mode => mode.name === 'Thoroughly')!.id;
    ...
    const orderData = [
        ...
        },
        {
            ...
            washingModeId: thoroughlyModeId,
            isSoak: true,
            paymentMethod: paymentMethods[0],
            price: 35000,
            authCode: '234567',
...
async function generatePowerUsageData() {
    // Fetch orders that are finished or confirmed with their washing modes
    ...
    const powerUsageData = completedOrders.map(order => ({
        ...
        totalKwh: order.washingMode.name === 'Normal'
...
async function main() {
    ...
    const washingModes = await generateWashingModes();
    await generateUsers();
    await generateWashingMachines();
    await generateOrders(washingModes);
